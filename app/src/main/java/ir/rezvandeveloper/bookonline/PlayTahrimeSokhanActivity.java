...
public class PlayTahrimeSokhanActivity extends AppCompatActivity {

    AppCompatTextView tv_name, tv_current_time, tv_total_time;

    AppCompatSeekBar sb_main;
    ImageButton ibBookmarkOffTahrimeSokhan,ibBookmarkOnTahrimeSokhan,ib_download_play, ib_play, ib_pause,ib_left_jump, ib_right_jump,ib_next_jump,ib_previous_jump;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_play_tahrime_sokhan);
        ...
        URL = Api.TAHRIME_SOKHAN_URL + CURRENT_NAME;
        ...
        ibBookmarkOffTahrimeSokhan.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ibBookmarkOffTahrimeSokhan.setVisibility(View.GONE);
                ibBookmarkOnTahrimeSokhan.setVisibility(View.VISIBLE);

                SharedPreferences.Editor editor = Api.sharedPreferences.edit();
                // TS == Tahrime Sokhan
                editor.putString("TS" + CURRENT_NAME,CURRENT_NAME);
                editor.apply();
            }
        });

        ibBookmarkOnTahrimeSokhan.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ibBookmarkOffTahrimeSokhan.setVisibility(View.VISIBLE);
                ibBookmarkOnTahrimeSokhan.setVisibility(View.GONE);

                SharedPreferences.Editor editor = Api.sharedPreferences.edit();
                // TS == Tahrime Sokhan
                editor.remove("TS" + CURRENT_NAME);
                editor.apply();
            }
        });
        ...
        ib_download_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                new Thread(new Runnable(){
                    @Override
                    public void run() {
                        if (ContextCompat.checkSelfPermission(PlayTahrimeSokhanActivity.this,WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                            requestPermission();
                        } else {
                            URL = Api.TAHRIME_SOKHAN_URL + CURRENT_NAME;
                            download(URL);
...
//    }
    ...
    private void readBookmarked(){
        // TS == Tahrime Sokhan
        if(Api.sharedPreferences.contains("TS" + CURRENT_NAME)) {
            ibBookmarkOnTahrimeSokhan.setVisibility(View.VISIBLE);
            ibBookmarkOffTahrimeSokhan.setVisibility(View.GONE);
        }else {
            ibBookmarkOnTahrimeSokhan.setVisibility(View.GONE);
    ...
    private void SaveLastNumber(String name) {
        SharedPreferences.Editor editor = Api.sharedPreferences.edit();
        editor.putString("nameTahrimeSokhan", name);
        editor.apply();
    }
    ...
    private String previous(String currentName){
        int lengthName = TahrimeSokhanActivity.listAllNamesTahrimeSokhan.size();
        String nameEnd = TahrimeSokhanActivity.listAllNamesTahrimeSokhan.get(0).getName();
        ...
        for (int i = lengthName ; i > 0 ; i--) {
            if (TahrimeSokhanActivity.listAllNamesTahrimeSokhan.get(i - 1).getName().equals(currentName)){
                return TahrimeSokhanActivity.listAllNamesTahrimeSokhan.get(i - 2).getName();
            }
        }
        return "";
    }
    ...
    private void play(String url){
        ...
        progressDialog = new ProgressDialog(PlayTahrimeSokhanActivity.this);
    ...
    private void increment(){
        ...
        RequestQueue queue = Volley.newRequestQueue(PlayTahrimeSokhanActivity.this);
    ...
    private void requestPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale
                (PlayTahrimeSokhanActivity.this, WRITE_EXTERNAL_STORAGE)) {
            reqPermission();
        } else {
            reqPermission();
        }
    ...
    private void reqPermission() {
        ActivityCompat.requestPermissions(PlayTahrimeSokhanActivity.this, new String[] {WRITE_EXTERNAL_STORAGE}, EXTERNAL_MEMORY_REQUEST_CODE);
    }
...
//                Toast.makeText(this, "مجوز رد شد .", Toast.LENGTH_SHORT).show();
    ...
    private void Cast() {
        ...
        tv_name = findViewById(R.id.tvNameTahrimeSokhan);
        tv_current_time = findViewById(R.id.tv_current_time_t_s);
        tv_total_time = findViewById(R.id.tv_total_time_t_s);

        sb_main = findViewById(R.id.sb_main_t_s);

        ib_play = findViewById(R.id.ib_play_t_s);
        ibBookmarkOffTahrimeSokhan = findViewById(R.id.ibBookmarkOffTahrimeSokhan);
        ibBookmarkOnTahrimeSokhan = findViewById(R.id.ibBookmarkOnTahrimeSokhan);
        ib_download_play = findViewById(R.id.ib_download_play1);
        ib_pause = findViewById(R.id.ib_pause_t_s);
        ib_left_jump = findViewById(R.id.ib_left_jump_t_s);
        ib_right_jump = findViewById(R.id.ib_right_jump_t_s);
        ib_next_jump = findViewById(R.id.ib_next_jump_t_s);
        ib_previous_jump = findViewById(R.id.ib_previous_jump_t_s);
