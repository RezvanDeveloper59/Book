...
import ir.rezvandeveloper.bookonline.OtherEslaheAlefbayeTohidActivity;
import ir.rezvandeveloper.bookonline.R;

public class PlayOtherEslaheAlefbayeTohidActivity extends AppCompatActivity {

    AppCompatTextView tv_name, tv_current_time, tv_total_time;

    AppCompatSeekBar sb_main;
    ImageButton ibBookmarkOff, ibBookmarkOn,ib_download_play, ib_play, ib_pause,ib_left_jump, ib_right_jump,ib_next_jump,ib_previous_jump;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_play_other_eslahe_alefbaye_tohid);
        ...
        URL = Api.OTHER_ESLAHE_ALEFBAYE_TOHID_URL + CURRENT_NAME;
        ...
        ibBookmarkOn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ibBookmarkOn.setVisibility(View.GONE);

                SharedPreferences.Editor editor = Api.sharedPreferences.edit();
                // TS == Tahrime Sokhan
                editor.remove("EAT" + CURRENT_NAME);
                editor.apply();
            }
        });
        ...
        ib_download_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                new Thread(new Runnable(){
                    @Override
                    public void run() {
                        if (ContextCompat.checkSelfPermission(PlayOtherEslaheAlefbayeTohidActivity.this,WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                            requestPermission();
                        } else {
                            URL = Api.OTHER_ESLAHE_ALEFBAYE_TOHID_URL + CURRENT_NAME;
                            download(URL);
...
//    }
    ...
    private void readBookmarked(){
        // TS == Tahrime Sokhan
        if(Api.sharedPreferences.contains("EAT" + CURRENT_NAME)) {
            ibBookmarkOn.setVisibility(View.VISIBLE);
            ibBookmarkOff.setVisibility(View.GONE);
        }else {
            ibBookmarkOn.setVisibility(View.GONE);
    ...
    private void SaveLastNumber(String name) {
        SharedPreferences.Editor editor = Api.sharedPreferences.edit();
        editor.putString("nameOtherEslaheAlefbayeTohid", name);
        editor.apply();
    }
    ...
    private String previous(String currentName){
        int lengthName = OtherEslaheAlefbayeTohidActivity.modelBookRvs.size();
        String nameEnd = OtherEslaheAlefbayeTohidActivity.modelBookRvs.get(0).getName();
        ...
        for (int i = lengthName ; i > 0 ; i--) {
            if (OtherEslaheAlefbayeTohidActivity.modelBookRvs.get(i - 1).getName().equals(currentName)){
                return OtherEslaheAlefbayeTohidActivity.modelBookRvs.get(i - 2).getName();
            }
        }
        return "";
    }
    ...
    private void play(String url){
        ...
        progressDialog = new ProgressDialog(PlayOtherEslaheAlefbayeTohidActivity.this);
    ...
    private void increment(){
        ...
        RequestQueue queue = Volley.newRequestQueue(PlayOtherEslaheAlefbayeTohidActivity.this);
    ...
    private void requestPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale
                (PlayOtherEslaheAlefbayeTohidActivity.this, WRITE_EXTERNAL_STORAGE)) {
            reqPermission();
        } else {
            reqPermission();
        }
    ...
    private void reqPermission() {
        ActivityCompat.requestPermissions(PlayOtherEslaheAlefbayeTohidActivity.this, new String[] {WRITE_EXTERNAL_STORAGE}, EXTERNAL_MEMORY_REQUEST_CODE);
    }
...
//                Toast.makeText(this, "مجوز رد شد .", Toast.LENGTH_SHORT).show();
    ...
    private void Cast() {
        ...
        tv_name = findViewById(R.id.tvNameOtherEslaheAlefbayeTohid);
        tv_current_time = findViewById(R.id.tv_current_time_t_sOtherEslaheAlefbayeTohid);
        tv_total_time = findViewById(R.id.tv_total_time_t_sOtherEslaheAlefbayeTohid);

        sb_main = findViewById(R.id.sb_main_t_sOtherEslaheAlefbayeTohid);

        ib_play = findViewById(R.id.ib_play_t_sOtherEslaheAlefbayeTohid);
        ibBookmarkOff = findViewById(R.id.ibBookmarkOffOtherEslaheAlefbayeTohid);
        ibBookmarkOn = findViewById(R.id.ibBookmarkOnOtherEslaheAlefbayeTohid);
        ib_download_play = findViewById(R.id.ib_download_play1OtherEslaheAlefbayeTohid);
        ib_pause = findViewById(R.id.ib_pause_t_sOtherEslaheAlefbayeTohid);
        ib_left_jump = findViewById(R.id.ib_left_jump_t_sOtherEslaheAlefbayeTohid);
        ib_right_jump = findViewById(R.id.ib_right_jump_t_sOtherEslaheAlefbayeTohid);
        ib_next_jump = findViewById(R.id.ib_next_jump_t_sOtherEslaheAlefbayeTohid);
        ib_previous_jump = findViewById(R.id.ib_previous_jump_t_sOtherEslaheAlefbayeTohid);
