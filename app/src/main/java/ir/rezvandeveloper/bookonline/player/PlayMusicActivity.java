...
import ir.rezvandeveloper.bookonline.MusicActivity;
...
public class PlayMusicActivity extends AppCompatActivity {
    ...
    ImageButton ib_download_play, ib_play, ib_pause,ib_left_jump, ib_right_jump,ib_next_jump,ib_previous_jump;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        URL = Api.MUSIC_LINK_URL + CURRENT_NAME;
        ...
        ib_download_play.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View arg0) {
                new Thread(new Runnable(){
                    @Override
                    public void run() {
                        if (ContextCompat.checkSelfPermission(PlayMusicActivity.this,WRITE_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
                            requestPermission();
                        } else {
                            URL = Api.MUSIC_LINK_URL + CURRENT_NAME;
                            download(URL);
...
//    }
    ...
    private String next(String currentName){
        int lengthName = MusicActivity.modelMusicRvs.size();

        String nameEnd = MusicActivity.modelMusicRvs.get(lengthName - 1).getName();
        ...
        for (int i = 0 ; i < lengthName ; i++) {
            if (MusicActivity.modelMusicRvs.get(i).getName().equals(currentName)){
                return MusicActivity.modelMusicRvs.get(i + 1).getName();
            }
        }
        return "";
    }
    ...
    private String previous(String currentName){
        int lengthName = MusicActivity.modelMusicRvs.size();
        String endName = MusicActivity.modelMusicRvs.get(0).getName();

        if (currentName.equals(endName)) {
            Toast.makeText(this, "تکرار شد .", Toast.LENGTH_LONG).show();
            return endName;
        }

        for (int i = lengthName ; i > 0 ; i--) {
            if (MusicActivity.modelMusicRvs.get(i - 1).getName().equals(currentName)){
                return MusicActivity.modelMusicRvs.get(i - 2).getName();
            }
        }
        return "";
    }
    ...
    private void play(String url){
        ...
        progressDialog = new ProgressDialog(PlayMusicActivity.this);
    ...
    private void increment(){
        ...
        RequestQueue queue = Volley.newRequestQueue(PlayMusicActivity.this);
    ...
    private void requestPermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale
                (PlayMusicActivity.this, WRITE_EXTERNAL_STORAGE)) {
            reqPermission();
        } else {
            reqPermission();
        }
    ...
    private void reqPermission() {
        ActivityCompat.requestPermissions(PlayMusicActivity.this, new String[] {WRITE_EXTERNAL_STORAGE}, EXTERNAL_MEMORY_REQUEST_CODE);
    }
...
//                Toast.makeText(this, "مجوز رد شد .", Toast.LENGTH_SHORT).show();
    ...
    private void Cast() {
        tv_name = findViewById(R.id.tvTitrMusic);
        tv_current_time = findViewById(R.id.tv_current_time_music);
        tv_total_time = findViewById(R.id.tv_total_time_music);
        sb_main = findViewById(R.id.sb_main_music);
        ib_download_play = findViewById(R.id.ib_download_play_music);
        ib_play = findViewById(R.id.ib_play_music);
        ib_pause = findViewById(R.id.ib_pause_music);
        ib_left_jump = findViewById(R.id.ib_left_jump_music);
        ib_right_jump = findViewById(R.id.ib_right_jump_music);
        ib_next_jump = findViewById(R.id.ib_next_jump_music);
        ib_previous_jump = findViewById(R.id.ib_previous_jump_music);
        timer = new Timer();
    }
}